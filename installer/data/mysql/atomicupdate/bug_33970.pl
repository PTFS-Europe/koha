...
return {
    ...
    up          => sub {
        ...
        if( !column_exists('illrequestattributes', 'backend') ) {
            ...
            $dbh->do(
                q{ ALTER TABLE illrequestattributes DROP FOREIGN KEY illrequestattributes_ifk }
            );

            say $out "Dropping primary key in 'illrequestattributes' table ";
            $dbh->do(
                q{ ALTER TABLE illrequestattributes DROP PRIMARY KEY }
            );

            say $out "Creating new primary key in 'illrequestattributes' table ";
            $dbh->do(
                q{ ALTER TABLE illrequestattributes ADD PRIMARY KEY( illrequest_id, backend, type (191)) }
            );
