if( CheckVersion( $DBversion ) ) {
    ...
    #  SELECT accountlines_id, id, accountlines.borrowernumber, debit_type_code, status, till, receiptid, paymenttype, date, timestamp, created FROM accountlines JOIN cash_transaction ON accountlines.date = date(cash_transaction.created) AND accountlines.debit_type_code = cash_transaction.tcode AND accountlines.amou
nt = cash_transaction.amt WHERE receiptid IS NOT NULL LIMIT 25;
