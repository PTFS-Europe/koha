    ...
    up          => sub {
        ...
        foreach my $account_key (@account_keys) {
            ...
            my $error_detect          = ( $SIPconfig->{accounts}->{$account_key}->{'error-detect'}
                    && $SIPconfig->{accounts}->{$account_key}->{'error-detect'} eq 'enabled' ) ? 1 : 0;
