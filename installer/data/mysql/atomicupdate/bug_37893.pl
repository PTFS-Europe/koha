...
return {
    ...
    up          => sub {
        ...
        foreach my $account_key (@account_keys) {
            ...
            my $sip_institution = Koha::SIP2::Institutions->find( { name => $SIPconfig->{accounts}->{$account_key}->{institution} } );
            next unless $sip_institution;

            my $insert_accounts = $dbh->prepare(
                q{INSERT IGNORE INTO sip_accounts (sip_institution_id, ae_field_template, allow_additional_materials_checkout, allow_empty_passwords, av_field_template, blocked_item_types, checked_in_ok, cr_item_field, ct_always_send, cv_send_00_on_success, cv_triggers_alert, da_field_template, delimiter, encoding, error_detect, format_due_date, hide_fields, holds_block_checkin, holds_get_captured, inhouse_item_types, inhouse_patron_categories, login_id, login_password, lost_status_for_missing, overdues_block_checkout, prevcheckout_block_checkout, register_id, seen_on_item_information, send_patron_home_library_in_af, show_checkin_message, show_outstanding_amount, terminator) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?)}
            );
            ...
            my $error_detect                = $SIPconfig->{accounts}->{$account_key}->{'error-detect'} eq 'enabled' ? 1 : 0;
            ...
            my $register_id                 = $SIPconfig->{accounts}->{$account_key}->{register_id};
