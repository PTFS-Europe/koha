...
return {
    ...
    up          => sub {
        ...
        if( !column_exists( 'erm_usage_data_providers', 'service_platform' ) ) {
          ...
          say_info( $out, "Added service_platform column to table 'erm_usage_data_providers'" );
        }
        say_success( $out, "Bug 37856 - Some SUSHI providers require the platform parameter - Done" );
    },
};
