...
subtest '_create_basket_for_file()' => sub {
    ...
    my $basket_id = Koha::MarcOrder::_create_basket_for_file(
        {
            filename  => $filename,
            vendor_id => $bookseller->id
        }
    );
    my $created_basket = Koha::Acquisition::Baskets->find($basket_id);
    is(
        $created_basket->basketno, $basket_id,
        "Basket created by NewBasket matches db basket"
    );
    is(
        $created_basket->basketname, $filename,
        "Basket created by NewBasket matches correct filename"
    );
    is(
        $created_basket->booksellerid, $bookseller->id,
        "Basket created by NewBasket matches correct vendor"
    );

    # Basket name should be truncated if longer than 50 characters
    my $long_basket_name = 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaabbbbb';
    my $truncated_version = 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa';
    my $basket_id2 = Koha::MarcOrder::_create_basket_for_file(
        {
            filename  => $long_basket_name,
            vendor_id => $bookseller->id
        }
    );
    my $created_basket2 = Koha::Acquisition::Baskets->find($basket_id2);
    is(
        $created_basket2->basketname, $truncated_version,
        "Basket created by NewBasket has a correctly truncated name"
    );
...
subtest 'add_biblios_from_import_record()' => sub {
    plan tests => 4;
    ...
    my $result = Koha::MarcOrder::add_biblios_from_import_record(
        {
    ...
    my $result2 = Koha::MarcOrder::add_biblios_from_import_record(
        {
            import_record             => $import_record,
            matcher_id                => $sample_import_batch->{matcher_id},
            overlay_action            => $sample_import_batch->{overlay_action},
...
subtest 'add_items_from_import_record()' => sub {
    plan tests => 9;
    ...
    my $budgetid2 = C4::Budgets::AddBudget(
        {
            budget_code      => "budget_code_test_2",
    ...
    is(
        @{$order_line_details}[0]->{quantity}, undef,
        "Quantity has been read correctly"
    );
    is(
...
replacementprice: 975$v'
    ...
    is($new_nb_of_orders, $quantity, 'Orders created successfully');
