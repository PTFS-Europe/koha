...
subtest 'list() tests' => sub {
    ...
    # Prepare some expected response structed
    my $request_status = {
        code =>"REQ",
        str =>"Requested"
    };
    ...
    $t->get_ok("//$userid:$password@/api/v1/ill_requests?q=$status_query" => {"x-koha-embed" => "+strings"} )
      ->status_is(200)
      ->json_is( [
    ...
    $t->get_ok("//$userid:$password@/api/v1/ill_requests?q=$status_alias_query" => {"x-koha-embed" => "+strings"} )
      ->status_is(200)
      ->json_is( [
                { _strings => { status => $response_status, status_av => $response_status_av }, %{$req_2->to_api} }
            ]
    ...
    $t->get_ok("//$userid:$password@/api/v1/ill_requests" => {"x-koha-embed" => "patron"} )
      ->status_is(200)
    ...
    $t->get_ok("//$userid:$password@/api/v1/ill_requests" => {"x-koha-embed" => "comments"} )
      ->status_is(200)
    ...
    $t->get_ok("//$userid:$password@/api/v1/ill_requests" => {"x-koha-embed" => "id_prefix"} )
      ->status_is(200)
    ...
    $t->get_ok( "//$userid:$password@/api/v1/ill_requests?request_blah=blah" )
      ->status_is(400)
      ->json_is(
    ...
    $t->get_ok(
        "//$unauth_userid:$password@/api/v1/ill_requests")
      ->status_is(403);

    # DELETE method not supported
    $t->delete_ok(
        "//$unauth_userid:$password@/api/v1/ill_requests/1")
      ->status_is(404);
