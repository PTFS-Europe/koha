...
sub check_bookings {
    ...
    while ( my $booking = $bookings->next ) {
        ...
        if ( defined($booking->itemnumber) && $booking->itemnumber == $self->itemnumber ) {
            # Might fulfil this booking, check patron after return
            $loanable_items = 0;
            last;
        }

        if ( !defined($booking->itemnumber) && $booking->borrowernumber == $params->{patron_id} ) {
            # This user booked it
            $loanable_items = 0;
            last;
        }
