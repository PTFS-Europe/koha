...
sub get_for_config {
    ...
    if (@custom_item_fields) {
        $unblessed->{custom_item_field} = scalar(@custom_item_fields) == 1 ? $custom_item_fields[0] : \@custom_item_fields;
    }
    ...
    if (@custom_patron_fields) {
        $unblessed->{custom_patron_field} = scalar(@custom_patron_fields) == 1 ? $custom_patron_fields[0] : \@custom_patron_fields;
    }
    ...
    my %system_preference_overrides = map { $_->variable => $_->value } @{ $self->system_preference_overrides->as_list };
