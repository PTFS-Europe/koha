...
if ( $template_name eq 'opac-results.tt' ) {
    $template->param(
        'COinSinOPACResults' => C4::Context->preference('COinSinOPACResults') );
}
...
if (   $format eq 'rss2'
    or $format eq 'opensearchdescription'
    or $format eq 'atom' )
{
...
$template->param(
    'OPACNoResultsFound' => C4::Context->preference('OPACNoResultsFound') );
...
$template->param( searchdomainloop => GetBranchCategories('searchdomain'), );
...
my $cnt;
my $advanced_search_types =
  C4::Context->preference("AdvancedSearchTypes") || "itemtypes";
...
if ( $yaml =~ /\S/ ) {
    ...
    eval { $hidingrules = YAML::Load($yaml); };
    if ($@) {
...
foreach my $advanced_srch_type (@advanced_search_types) {
    ...
    if ( $advanced_srch_type eq 'itemtypes' ) {
       ...
       # itemtype is a special case, since it's not defined in authorized values
        ...
        {
            next
              if $hidingrules->{itype} && any { $_ eq $thisitemtype }
            @{ $hidingrules->{itype} };
            next
              if $hidingrules->{itemtype} && any { $_ eq $thisitemtype }
        ...
        my $advsearchtypes =
          GetAuthorisedValues( $advanced_srch_type, '', 'opac' );
        my @authvalueloop;
        for my $thisitemtype (@$advsearchtypes) {
            my $hiding_key = lc $thisitemtype->{category};
            $hiding_key = "location" if $hiding_key eq 'loc';
            next
              if $hidingrules->{$hiding_key}
              && any { $_ eq $thisitemtype->{authorised_value} }
...
if ( $template_type && $template_type eq 'advsearch' ) {
    ...
    my @advsearch_limits = split /,/,
      C4::Context->preference('OpacAdvSearchOptions');
    my @advsearch_more_limits = split /,/,
      C4::Context->preference('OpacAdvSearchMoreOptions');
    $template->param(
        uc( C4::Context->preference("marcflavour") ) =>
          1,    # we already did this for UNIMARC
        advsearch                => 1,
...
(
  ...
  = buildQuery( \@operators, \@operands, \@indexes, \@limits, \@sort_by, 0,
    $lang );
...
if ($tag) {
    ...
    my @marclist = (
        map {
            ( C4::Context->config('zebra_bib_index_mode') eq 'dom' )
              ? $_->{marcxml}
              : $_->{marc};
        } @biblist
    );
    $DEBUG
      and printf STDERR "taglist (%s biblionumber)\nmarclist (%s records)\n",
   ...
   # FIXME: No facets for tags search.
    ...
    eval {
        ( $error, $results_hashref, $facets ) = C4::Search::pazGetRecords(
    ...
    $pasarParams .= '&amp;query_type=' . uri_escape($query_type)
      if ($query_type);
    eval {
        ( $error, $results_hashref, $facets ) = getRecords(
            $query,          $simple_query,     \@sort_by,
            \@servers,       $results_per_page, $offset,
            $expanded_facet, $branches,         $itemtypes,
            $query_type,     $scan,             1
        );
    };
}
...
if ($facets) {
    foreach my $f (@$facets) {
        $f->{facets} = [
            sort {
                uc( $a->{facet_title_value} ) cmp uc( $b->{facet_title_value} )
            } @{ $f->{facets} }
        ];
    }
...
for ( my $i = 0 ; $i < @servers ; $i++ ) {
    my $server = $servers[$i];
    if ( $server && $server =~ /biblioserver/ )
    {    # this is the local bibliographic server
        ...
        if ($build_grouped_results) {
            foreach my $group ( @{ $results_hashref->{$server}->{"GROUPS"} } ) {
                ...
                my @group_results =
                  searchResults( 'opac', $query_desc, $group->{'group_count'},
                    $results_per_page, 0, $scan, $group->{"RECORDS"} );
                push @newresults,
                  {
            ...
            @newresults =
              searchResults( 'opac', $query_desc, $hits, $results_per_page,
                $offset, $scan, $results_hashref->{$server}->{"RECORDS"} );
        }
        ...
        foreach my $res (@newresults) {
          ...
          # in order to avoid problems generated by the default size value in TT
                    ...
                    if ( C4::Context->preference("Babeltheque") ) {
                        ...
                        if ($social_datas) {
                            for my $key ( keys %$social_datas ) {
                                ...
                                if ( $key eq 'score_avg' ) {
                                    $res->{score_int} = sprintf( "%.0f",
                                        $$social_datas{score_avg} );
                                }
                            }
                        }
                    }
            ...
            if ( C4::Context->preference('OpacStarRatings') eq 'all' ) {
                my $rating =
                  GetRating( $res->{'biblionumber'}, $borrowernumber );
        ...
        if ( C4::Context->preference('EnableOpacSearchHistory') ) {
            unless ($offset) {
                ...
                my $query_desc_history = join ", ",
                  grep { defined $_ } $query_desc, $limit_desc;
            ...
            && $format ne 'atom' )
        {
            ...
            if ( C4::Context->preference('BiblioDefaultView') eq 'isbd' ) {
                print $cgi->redirect(
"/cgi-bin/koha/opac-ISBDdetail.pl?biblionumber=$biblionumber"
                );
            }
            elsif ( C4::Context->preference('BiblioDefaultView') eq 'marc' ) {
                print $cgi->redirect(
"/cgi-bin/koha/opac-MARCdetail.pl?biblionumber=$biblionumber"
                );
            }
            else {
                print $cgi->redirect(
...
# We build the encrypted list of first OPACnumSearchResults biblios to pass with the search criteria for paging on opac-detail
                ...
                foreach (@newresults) {
                    my $bibnum =
                      ( $_->{biblionumber} ) ? $_->{biblionumber} : 0;
                    $pasarParams .= uri_escape($bibnum) . ',';
                    $j++;
                    last if ( $j == $results_per_page );
                }
                chop $pasarParams if ( $pasarParams =~ /,$/ );
                $pasarParams .= '&amp;total=' . uri_escape( int($total) )
                  if ( $pasarParams !~ /total=(?:[0-9]+)?/ );
                if ($pasarParams) {
            ...
            $limit_cgi_not_availablity =~ s/&limit=available//g
              if defined $limit_cgi_not_availablity;
            $template->param(
                limit_cgi_not_availablity => $limit_cgi_not_availablity );
            ...
            $hide = ( $hide =~ m/\S/ )
              if $hide;    # Just in case it has some spaces/new lines
            my $branch = '';
            ...
            if ( C4::Context->preference('HighlightOwnItemsOnOPAC') ) {
                    ...
                    (
                        (
                            C4::Context->preference(
                                'HighlightOwnItemsOnOPACWhich') eq
                            'PatronBranch'
                        )
                        && $branch
                    )
                  ...
                  )
                {
                    ...
                    {
                        $branchname =
                          $branches->{ $ENV{'BRANCHCODE'} }->{'branchname'};
                    }
            ...
            if ( C4::Context->preference("OPACLocalCoverImages") ) {
                ...
                $template->param( OPACLocalCoverImagesPriority =>
                      C4::Context->preference("OPACLocalCoverImagesPriority") );
            }
            ...
            # now, show twenty pages, with the current one smack in the middle
                ...
                for (
                    ...
                    $i <= ( $current_page_number + 20 ) ;
                    $i++
                  )
                {
                    my $this_offset = ( ( ( $i - 9 ) * $results_per_page ) -
                          $results_per_page );
    ...
    elsif ( $server && $server =~ /authorityserver/ )
    {    # this is the local authority server
...
my @input_values =
  map { Encode::decode_utf8( $_->{input_value} ) } @limit_inputs;
for my $facet (@$facets) {
...
if ( $offset == 0 && IsOverDriveEnabled() ) {
    ...
    $template->param(
        OverDriveLibraryID => C4::Context->preference('OverDriveLibraryID') );
}
