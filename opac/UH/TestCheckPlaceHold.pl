...
foreach my $rec (@$data) {
	my ($availabilty,$numberOfHolds) = @$rec;
	if ($availability == 'Available') {
		$available++;
		$debugString .= "<p>Incrementing availability variable by one to: " . $available . "</p>\n";
	}
	$holdCount = $numberOfHolds;
	$debugString .= "<p>The hold count is " . $holdCount . "</p>\n";
}

if ($available > 0) {
	$debugString .= "<p>Available variable is greater than zero " . $available . "</p>\n";
	if (($available - $holdCount) > 0) {
		$debugString .= "<p>" . $available . " minus " . $holdCount . " is greater than zero</p>\n";
		$showLink = 0;
	} else {
		$debugString .= "<p>" . $available . " minus " . $holdCount . " is <b>NOT</b> greater than zero</p>\n";
		$showLink = 1;
	}
} else {
	$showLink = 1;
