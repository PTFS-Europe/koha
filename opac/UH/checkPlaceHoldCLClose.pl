...
my $available = 0;
my $holdCount = 0;
# if we have the same item at both campuses
# then maybe we need to do a count, and show
# link if any CL items?
my $LRC;

my $showLink = 0;

foreach my $rec (@$data) {
	my ($availability,$numberOfHolds,$waiting,$homebranch) = @$rec;
...
$debugString .= "<br>The LRC is $LRC <br>\n";

if ($LRC eq 'CL') { $showLink = 2; } else {
