...
foreach my $rec (@$data) {
	my ($id,$loc,$callnumber,$availabilty,$itemtype,$onorder) = @$rec;
	...
	$xmlbody .= "<itemdetails>\n";
	if ($onorder eq "onorder") {
		$availabilty = "On Order";
	}
	$xmlbody .= "<status>$availabilty</status>\n";
	if ($itemtype eq "REF") {
		$xmlbody .= "<location>$loc (Reference)</location>\n";
	} else {
...
sub getAvailability($) {
	...
	my $SQL = "select
				...
				itype,
				if(items.notforloan = '-1','onorder',items.notforloan) notforloan
