...
# @deps:	(none)
load_default_config() {
  CONFIG_configfile="/home/koha/.automysqlbackup/automysqlbackup.conf"
  CONFIG_backup_dir="/home/koha/backup"
	    ...
	    'files')
			    ...
			    if (( ${CONFIG_mail_maxattsize} >= ${attsize} )); then
					...
					if sPWD="$PWD"; cd "$CONFIG_backup_dir"/tmp && pax -wv "${backupfiles[@]}" | bzip2_compression | split -b $((CONFIG_mail_maxattsize*1000)) - mail_attachment_${datetimestamp}_ && cd "$sPWD"; then
					  ...
					  cat "$log_file" | mail -s "WARNING: Backup exceeds max attachment size on ${CONFIG_mysql_dump_host_friendly:-$CONFIG_mysql_dump_host} - ${datetimestamp}" ${CONFIG_mail_address}
					fi
				else
				    cat "$log_file" | mail -s "WARNING: Backup exceeds max attachment size on ${CONFIG_mysql_dump_host_friendly:-$CONFIG_mysql_dump_host} - ${datetimestamp}" ${CONFIG_mail_address}
			    fi
			    ...
			    ;;
	    'log')
			    cat "$log_file" | mail -s "SUCCESS: MySQL Backup Log for ${CONFIG_mysql_dump_host_friendly:-$CONFIG_mysql_dump_host} - ${datetimestamp}" ${CONFIG_mail_address}
			    [[ -s "$log_errfile" ]] && cat "$log_errfile" | mail -s "ERRORS: MySQL Backup error Log for ${CONFIG_mysql_dump_host_friendly:-$CONFIG_mysql_dump_host} - ${datetimestamp}" ${CONFIG_mail_address}
			    ;;
	    'quiet')
			    [[ -s "$log_errfile" ]] && cat "$log_errfile" | mail -s "ERRORS: MySQL Backup error Log for ${CONFIG_mysql_dump_host_friendly:-$CONFIG_mysql_dump_host} - ${datetimestamp}" ${CONFIG_mail_address}
			    ;;
...
method_backup () {
	...
	echo "======================================================================"
	echo "AutoMySQLBackup version ${version}.PTFS"
