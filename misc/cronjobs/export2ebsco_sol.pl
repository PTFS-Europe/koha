...
sub write_file {
    ...
    my $query =
      #'SELECT distinct biblioitems.biblionumber FROM biblioitems LEFT JOIN items ON (biblioitems.biblionumber = items.biblionumber) WHERE biblioitems.biblionumber >0 AND items.homebranch IN ("HIC", "BAR")  ';
      'SELECT distinct biblioitems.biblionumber FROM biblioitems LEFT JOIN items ON (biblioitems.biblionumber = items.biblionumber) WHERE biblioitems.biblionumber >0 ';
...
sub get_filename {
    ...
    my $month = $tm[4];
    my $year  = $tm[5] - 100;
    my $name  = 'sol-export' . $abbr[$month] . $year;
...
sub transfer_file {
    ...
    my $username  = q(cat05660a);
    my $password = q(^dPOId?3r3);
